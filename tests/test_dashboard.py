...
def test_dashboard_metrics(dashboard_setup):
    ...
    pytest.skip(
        "Skipping dashboard test: relies on Postgres specific functions not available in SQLite"
    )
    ...
    eur_total = next(
        (item for item in data["totals_by_currency"] if item["currency"] == "EUR"), None
    )
